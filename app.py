...
def hyper_accurate_disease_prediction(data, biomarkers):
    X = torch.tensor(data[biomarkers].values, dtype=torch.float32)
    y = torch.tensor(data['label'].values, dtype=torch.float32).view(-1, 1)
    ...
    ax.plot(y.numpy(), label='Actual Label', marker='x', linestyle='--')
